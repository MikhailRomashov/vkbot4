<?php
// склад запросов к mysql дл€ более легкого изменени€
class MysqlQueryVkbotV4
{
  ...
  public function __construct(int $bot_id,string $sname)
  {
     ...
     try
      ...
      {
          $this->Log->save("ErrMySQL_",__LINE__,$e->getMessage());
          die;
      }

      // работаем только с данным ботнетом
      $this->botnet_num=$this->sql_robots_select0()[botnet_num];
  }
  ...
  public function sql_azenv_select($status=0,$limit=0)
  {
   ...
   // выбираем из функции query_warehouse запрос одноименный вызыаемой  функции и отправл€ем с параметрами на выполнение в фугкцию execute
 ...
 // проверить как рабоатета LIKE ':ip1%'
  ...
  // проверить встраиваетс€ ли строка $tk=",line='0'"
  ...
  public function  sql_robots_update3()
 {
  ...
  public function  sql_robots_update3_1(int $line,int $url_timeout)
  {
   return $this->execute($this->query_warehouse(__FUNCTION__),array('bot_id' => $this->bot_id,'line' => $line,'url_timeout' => $url_timeout));
  }
  ...
  private function query_warehouse($req)
  {
    ...
    switch ($req) {
        ...
        //////////////////
         ...
         //groups_invite_wait
         //////////////////
         case "sql_groups_invite_wait_select1": $s = "select * from groups_invite_wait where bot_id=':bot_id' and group_vk_id=':roup_vk_id' limit 1"; break;
        ...
        ////////////////////
         ...
         case "sql_anons_suffix_select1": $s = "select SUBSTRING(MAX(CONCAT(TRUNCATE(RAND(),4),text)),7) AS text FROM anons_suffix where anons_num=':suffix_num' GROUP BY phrase_id"; break;
          ...
          //message_in
          //////////////////
         case "sql_message_in_select1": $s = "SELECT id,answer_id FROM message_in WHERE text=':hel_mes' LIMIT 1"; break;
         ...
         default : // обработка ошибки
             $this->Log->save("ErrMySQL_",__LINE__,"неверный запрос $s из скрипта ".debug_backtrace()[0]["script"]." строки ".debug_backtrace()[0]["line"]);
    }
   return $s;
  }

  // функци€ исполнени€ запроса
