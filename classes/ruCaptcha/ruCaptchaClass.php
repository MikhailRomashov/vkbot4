...
class ruCaptchaClass
{
    ...
    {
        $this->action = 0; // сколько раз выполнялись функции которые могут запросить подтверждение капчей
        $this->captcha = 0; // сколько раз запросили капчу в таких фукнциях
    }

    public function recognize($url,$captcha_id)
    {
    ...
    public function reportbad($captcha_id)
    {
        $url="";
        $apikey='022dfafd50710dc8ad24dd91f6b01883';
        $domain="rucaptcha.com";
    ...
    public function TooMuchCaptchaReaction()
    {
        $this->captcha++;

        if ($this->captcha > 2 && ($this->captcha / $this->action) > 0.4) return true;
