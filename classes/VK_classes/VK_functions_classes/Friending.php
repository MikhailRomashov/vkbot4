            ...
            if($add_friend_link[status]===true)
            {
                ...
                if($add_friend_link_decline[status]===true)
                {
                    ...
                    if($add_friend_link_decline[html]=='accept')
                    {
                        ...
                        do {
                            ...
                            if(!$captcha_form[status])
                            {
                                ...
                                if($cap_zavis)
                                {
                                    ...
                                    {
                                        $this->GenFunc->Log->save("ChangePhone", __LINE__ ,". запросов капчи:" . $this->GenFunc->Captcha->captcha . ". выполнено функций " . $this->GenFunc->Captcha->action . " \r\n");
                            ...
                            if ($captcha_form[params][name][1] == 'captcha_sid')
                            {
                                ...
                                if($cap_zavis>0)
                                {
                                    ...
                                    $this->GenFunc->Log->save("RepostCapErr", __LINE__, " ошибка распочнания капчи при репосте. Форма:\r\n" . var_export($captcha_form, true) . " \r\n postdata:\r\n" . var_export($postdata, true) . " \r\n");
                                ...
                                if($captcha_key==false)
                                {
                                    ...
                                    $this->GenFunc->Services->reboot(1); // перезапуск бота
                                }
                                ...
                                $this->GenFunc->Log->save("CAPlogFrAdd",__LINE__," Капча при заявке в друзья. Форма:\r\n".var_export($captcha_form,true)." \r\n postdata:\r\n".var_export($postdata,true));
                            }
                            else
                            {
                        ...
                        $this->GenFunc->Log->save("FrAddCapErr", __LINE__, " завис при распознании капчи cap_zavis=$cap_zavis \r\n");
                        die;
